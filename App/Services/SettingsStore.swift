...
final class SettingsStore: ObservableObject {
    ...
    /// #vpn: which backend `TunnelManager.connect` starts a session through —
    /// the in-app SOCKS proxy (default) or the system packet tunnel. Read once
    /// per session at connect time; switching is gated on `.disconnected` in
    /// the Config tab, so a live session never changes backend.
    @Published var tunnelMode: TunnelMode {
        didSet { Self.persist(tunnelMode.rawValue, forKey: Keys.tunnelMode) }
    }
