...
What it does:
  1. reads MARKETING_VERSION + CURRENT_PROJECT_VERSION from project.yml -> tag v<mkt>.<build>
...
def project_version() -> tuple[str, str]:
    ...
    # #vpn: the build number is hoisted to top-level settings.base as
    # CURRENT_PROJECT_VERSION (shared by the app and the tunnel appex); keep
    # the old CFBundleVersion literal as a fallback for pre-hoist layouts.
    bld = (re.search(r"CURRENT_PROJECT_VERSION:\s*[\"']?(\d+)", txt)
           or re.search(r"CFBundleVersion:\s*[\"']?(\d+)", txt))
    if not mkt or not bld:
        sys.exit("error: MARKETING_VERSION / CURRENT_PROJECT_VERSION not found in project.yml")
...
def main() -> None:
    ...
    if git("rev-parse", "-q", "--verify", f"refs/tags/{tag}", check=False).returncode == 0:
        sys.exit(f"error: tag {tag} already exists — bump CURRENT_PROJECT_VERSION in project.yml first.")
