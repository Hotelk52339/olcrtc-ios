...
olcrtc-upstream/install.sh (upstream) — in BOTH directions (#325), and
...
----------------------
install.sh's cnc (client) mode: runs the olcrtc CLI binary in a container on
Linux/macOS. On iOS the client is Mobile.xcframework (gomobile bindings), so
there is no 1-to-1 correspondence to compare — install.sh's cnc-mode lines
live in `# boc olcrtc-ios-rejected:` blocks in scripts/srv.sh (existence-only
...
# Master deleted script/srv.sh; the server installer now lives in install.sh
# (interactive, srv+cnc dual-mode). scripts/srv.sh is our non-interactive,
# server-mode-only patched copy of it.
UPSTREAM = os.path.join(SRCROOT, "olcrtc-upstream", "install.sh")
...
print("note: [parity] Checking scripts/srv.sh vs olcrtc-upstream/install.sh ...")
...
if drift_errors:
    ...
    print("       Next step:  diff -u olcrtc-upstream/install.sh scripts/srv.sh | less")
    print()
